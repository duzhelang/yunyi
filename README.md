...
│   ├── incremental_train.py # 增量训练脚本（微调）
│   ├── model.py           # 模型定义
│   ├── predict.py         # 预测脚本
│   ├── read_data.py       # 数据读取工具
│   └── train.py           # 模型训练脚本
...
### 4. 在线训练
- 支持新建模型训练（train.py）
- 支持增量训练/微调（incremental_train.py）
- 多数据集选择（V1.0、扩展、历史）
- 可视化训练指标（准确率、损失、精确率、召回率、F1、AUC）
- 混淆矩阵展示
- 训练任务历史记录
...
### 模型架构
- 三层全连接神经网络（fc1 → fc2 → fc3）
- Dropout 正则化（0.2）
- ReLU 激活函数

### 训练模式
1. **新建训练**（train.py）：从头训练新模型
2. **增量训练**（incremental_train.py）：基于已有模型微调
   - 支持加载已有模型权重
   - 可选复用 scaler/encoder 预处理器
   - 可选冻结部分层（fc1、fc1+fc2、all-hidden）
...
### 模型训练
1. 准备训练数据（CSV格式，放入 `python/data/train/` 目录）
2. 执行训练脚本：`python python/train.py <输入CSV路径> <模型输出路径>`
3. 增量训练：`python python/incremental_train.py <输入CSV路径> <输出路径> --base-model <基础模型路径>`
4. 模型将保存为 `.pth` 文件，同时生成编码器和缩放器文件
5. 训练指标以 JSON 格式输出到 stdout
