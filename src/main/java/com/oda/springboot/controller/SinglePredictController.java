...
public class SinglePredictController {
        ...
        try {
            ...
            if ("success".equals(status)) {
                ...
                if (resultNode.has("confidence_interval")) {
                    resultData.put("confidence_interval", mapper.convertValue(
                        resultNode.get("confidence_interval"), double[].class));
                }
                if (resultNode.has("feature_names")) {
                    resultData.put("feature_names", mapper.convertValue(
                        resultNode.get("feature_names"), String[].class));
                }
                if (resultNode.has("feature_importance")) {
                    resultData.put("feature_importance", mapper.convertValue(
                        resultNode.get("feature_importance"), double[].class));
                }
                if (resultNode.has("percentiles")) {
                    resultData.put("percentiles", mapper.convertValue(
                        resultNode.get("percentiles"), Map.class));
                }
                if (resultNode.has("similar_cases")) {
                    resultData.put("similar_cases", mapper.convertValue(
                        resultNode.get("similar_cases"), Map.class));
                }
                if (resultNode.has("charts")) {
                    resultData.put("charts", mapper.convertValue(
     ...
     * 验证输入参数的有效性范围
     */
    private boolean validateInput(double pregnancies, double glucose, double bloodPressure, 
     ...
     * 带超时控制的 Python 脚本调用
     */
    private String callPythonWithTimeout(final String[] arguments, int timeoutSeconds) 
